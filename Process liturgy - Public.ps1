...
$CrossWayV3APIKey = 'replacethiswithyourapikey' #You must create your own api key here: https://api.esv.org/account
...
Function ValidateLiturgy{
    ...
    if(!$liturgyUnStructuredText){
        ...
        ForEach($row in $arrLiturgy){
            ...
            if(($matchedrow -eq $false) -and ((($previousItem.Type -eq 'Reading') -and $row -match "^\s+" -and ($rowWordSplit | Where{$_ -in ($validReadings.keys)})) -or $row -match 'Reading' -or $row -match 'Scripture' -or $row -match 'Sermon' -or $row -match 'Text' -or $row -match 'Read' -or $row -match 'Cat' -or ($rowWordSplit | Where{$_ -in ($validReadings.keys | where{$validReadings.$_ -ne 'Ps.' -and ($row -notmatch 'sing') -and ($row -notmatch 'preservice song') -and ($row -notmatch 'pre-service song') -and ($row -notmatch '(?<!Song\sof\s)(?i)(song)(?!\sof\sSo)'<#Dont match the word song if it isn't part of Song of So#>)})}))){ #($rowWordSplit | Where{$_ -in $validReadings.keys}) this may be problematic if the theme contains the word 'Psalm' or songs?
            #Reading
...
Function Get-BibleText{
    Param(
        $TextToLookUp = 'Gen 1:1'          
    )
    $url = 'https://api.esv.org/v3/passage/text/'
    $queryParams = @{
                    'q'=$TextToLookUp
    ...
    $jsonResponse = Invoke-RestMethod -Method Get -Uri $url -Headers $headers -Body $queryParams -UseBasicParsing
    ([string]($jsonResponse.passages) -Replace "(\r|\n|\t|\s{2,})+", ' ').TrimStart(' ').TrimEnd(' ')
}           
